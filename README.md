...
- **`pkg/execution`**: Unified execution contract (`Place`/`Cancel`/`Query`/`Replay`), CLOB adapter, normalized lifecycle model (`created`/`accepted`/`partial`/`filled`/`canceled`/`rejected`), and deterministic idempotency-key spec (`tenant + strategy + client_order_id`).
